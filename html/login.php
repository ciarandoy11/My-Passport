        ...
        if (password_verify($inputPassword, $hashedPassword)) {
            ...
            if (payment_verify($userId, $conn)) {
                if (subscription_tier($userId, $conn)) {
                    if ($typeAdmin == "1") {
                        header("Location: /early_access/admin");
                        exit();
                    } elseif ($typeCoach == "1") {
                        header("Location: /early_access/coachs-site");
                        exit();
                    } else {
                        header("Location: /early_access/dashboard");
                        exit();
                    }
                    $updateSql = "UPDATE users SET subscription_tier = 'earlyAccess' WHERE id = ?";
                    ...
                    if ($typeAdmin == "1") {
                        ...
                        header("Location: /coachs-site");
                        exit();
                    } else {
                        header("Location: /dashboard");
                        exit();
                    }
                    $updateSql = "UPDATE users SET subscription_tier = 'basic' WHERE id = ?";
                    $updateStmt = $conn->prepare($updateSql);
                    $updateStmt->bind_param("i", $userId);
                    $updateStmt->execute();
                    $updateStmt->close();
