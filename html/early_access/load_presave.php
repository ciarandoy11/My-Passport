...
$club = $user['club'];
        ...
        if (json_last_error() === JSON_ERROR_NONE) {
            ...
            foreach ($sessionDataArray as &$session) {
                ...
                $stmt = $conn->prepare("INSERT INTO timetable (_day, _time, stime, etime, _group, _location, otherActivity, club) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                if ($stmt === false) {
                    die(json_encode(["status" => "error", "message" => "Prepare failed: " . $conn->error]));
                }
                ...
                $otherActivity = $session['otherActivity'] ? $session['otherActivity'] : 0;
                $club = $club;

                $stmt->bind_param("ssssssis", $dayWithNewDate, $time, $stime, $etime, $group, $location, $otherActivity, $club);
                if (!$stmt->execute()) {
