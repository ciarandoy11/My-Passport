...
<head>
        ...
        <?php if ($typeAdmin === 1) {
            ...
            <div class="clearfix">
                <div class="sidebar">
                    <button class="dropdown-toggle" onclick="toggleMenu()" id="menuButton">Menu ☰</button>
                    <ul id="dropdown-menu">
                        <li data-page="beethoven"><a href="?page=beethoven">Compose email</a></li>
                        <li data-page="inbox"><a href="?page=inbox">Inbox</a></li>
                        <li data-page="sent"><a href="?page=sent">Sent</a></li>
                        <li data-page="drafts"><a href="?page=drafts">Drafts</a></li>
                        <li data-page="trash"><a href="?page=trash">Trash</a></li>
                        <li data-page="spam"><a href="?page=spam">Spam</a></li>
                    </ul>
                </div>
                ...
                <script>
                    ...
                    function getCurrentPageName() {
                        ...
                        return map[page.toLowerCase()] || "Inbox";
                    }
                
                    // Set current page name as dropdown label
                    const currentPageText = getCurrentPageName();
                    document.getElementById("menuButton").innerText = `${currentPageText} ☰`;
                
                    // Highlight active item
                    const currentPage = new URLSearchParams(window.location.search).get("page");
                    if (currentPage) {
                        const currentItem = document.querySelector(`li[data-page="${currentPage}"]`);
                        if (currentItem) {
        ...
        <div class="inbox">
            ...
            <?php } elseif ($page === 'beethoven') {
			   ...
			   $stmt = $conn->prepare("SELECT email FROM users WHERE email = ?");
                ...
                // Specific user invoices and payments section
...
// Attempt to fetch Stripe invoices if Stripe API key is available
if (isset($clubStripeApi)) {
    if (!empty($customerId)) {
        try {
            $customer = \Stripe\Customer::retrieve($customerId); // Fetch the customer info before fetching invoices
            $invoices = \Stripe\Invoice::all(['customer' => $customerId]); // Fetch the invoices for the specific customer
        } catch (\Stripe\Exception\ApiErrorException $e) {
            echo 'Error fetching Stripe invoices: ' . htmlspecialchars($e->getMessage());
        }
    } else {
        echo 'Error fetching Stripe invoices: Invalid customer ID.';
    }
}

// Attempt to fetch GoCardless payments if GoCardless API key is available
if (isset($clubGoCardlessApi)) {
    if (!empty($customerId)) {
        try {
            $customer = $client->customers()->get($customerId); // Fetch the customer info before fetching payments
            $payments = $client->payments()->list(['customer' => $customerId]); // Fetch the payments for the specific customer
        } catch (\GoCardlessPro\Core\Exception\ApiException $e) {
            echo 'Error fetching GoCardless payments: ' . htmlspecialchars($e->getMessage());
        }
    } else {
        echo 'Error fetching GoCardless payments: Invalid customer ID.';
    }
}

// Check which data is available and handle accordingly
if (isset($invoices) && $invoices->data) {
    // Handle Stripe invoices
    foreach ($invoices->data as $invoice) {
        $invoiceDate = date('Y-m-d', $invoice->created);
        $subject = 'Unpaid Invoice';
        $message = 'Hi ' . $userfname . ',<br><br>We have noticed that your invoice sent by us on ' . $invoiceDate . ', is still unpaid.<br><br>Please pay <a href="' . htmlspecialchars($invoice->hosted_invoice_url) . '">here</a> or complete this form explaining the missing payment: ' . htmlspecialchars($formlink) . '<br>Yours Sincerely, ' . $firstname . '.';
    }
}
}
?>
...
<?php } elseif ($page === 'viewEmail') {
    ...
    if ($subject) {
		...
		header("Location: /clubEmails?page=inbox");
    }
}
?>
