...
<?php include 'includes/admin_navigation.php'; ?>
    ...
    if ($ci === 'True' && !empty($clubSecrets)) { // Check both conditions
        ...
        echo '<div class="back-link">
                <button onclick="window.location.href=\'clubMembership\'"><i class="fa fa-arrow-left"></i> Back to All members</button>
              </div>';
                    ...
                    if (count($invoices->data) > 0) {
                        ...
                        foreach ($invoices->data as $invoice) {
                            if ($invoice->status === 'paid' || $invoice->status === 'void') {
                                ...
                                echo '<tr style="background-color: red;" onclick="document.location=\'/clubEmails?clubInvoiceUserEmail=' . htmlspecialchars($customer->email) . '&clubInvoiceUserID=' . htmlspecialchars($customer->id) . '&page=beethoven&invoiceNumber=' . $invoice->id . '\'">';
                            } elseif ($invoice->status === 'draft') {
                                echo '<tr style="background-color: orange;">';
                            } else {
                                echo '<tr onclick="document.location=\'/clubEmails?clubInvoiceUserEmail=' . htmlspecialchars($customer->email) . '&clubInvoiceUserID=' . htmlspecialchars($customer->id) . '&page=beethoven&invoiceNumber=' . $invoice->id . '\'">';
                            }
                            echo '<td>' . htmlspecialchars($invoice->id) . '</td>';
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.payment_method === 'stripe') {
                        ...
                        window.location.href = `stripe_payment?payment_intent=${data.payment_id}`;
                    } else if (data.payment_method === 'goCardless') {
                        // Redirect to GoCardless payment page
                        window.location.href = `gocardless_payment?payment_id=${data.payment_id}`;
                    } else {
