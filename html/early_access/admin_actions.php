...
switch ($action) {
    ...
    case 'get_other_activity_athletes':
        getOtherActivityAthletes($conn, $club);
        break;

    case 'get_activity_type':
        $id = isset($_POST['id']) ? $_POST['id'] : '';
        getActivityType($conn, $id);
        break;

    case 'get_date_exemption':
        $date = isset($_POST['date']) ? $_POST['date'] : '';
        getDateExemption($conn, $date, $club);
        break;
}
...
function handlePostRequest($conn, $club) {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'delete_item') {
        ...
        // Do nothing
	} elseif (isset($_POST['action']) && $_POST['action'] === 'get_other_activity_athletes') {
        getOtherActivityAthletes($conn, $club);
    } elseif (isset($_POST['action']) && $_POST['action'] === 'get_activity_type') {
        getActivityType($conn, $id);
    } else {
...
function getOtherActivityAthletes($conn, $club) {
    $sql = "SELECT item_name FROM groups WHERE otherActivity = 1 AND club = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $club);
    $stmt->execute();
    $result = $stmt->get_result();
    $otherActivityAthletes = [];
    while ($row = $result->fetch_assoc()) {
        $otherActivityAthletes[] = $row['item_name'];
    }
    echo json_encode(['otherActivityAthletes' => $otherActivityAthletes]);
    exit;
}

function getActivityType($conn, $id) {
    $sql = "SELECT otherActivity FROM timetable WHERE id = ?";
    $stmt = $conn->prepare($sql);
    ...
    $activityType = ($row['otherActivity'] ?? 0) == 1 ? 'Other' : 'Training';

    echo json_encode(['activityType' => $activityType]);
    exit;
}

// Function to get date exemptions for a group on a specific day
function getDateExemption($conn, $date, $club) {
    $sql = "SELECT item_name FROM groups WHERE FIND_IN_SET(?, pod_date_exemption) AND club = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ss", $date, $club);
    $stmt->execute();
    $result = $stmt->get_result();
    $dateExemption = [];
    while ($row = $result->fetch_assoc()) {
        $dateExemption[] = $row['item_name'];
    }
    echo json_encode(['dateExemption' => $dateExemption]); 
    exit;
}
?>