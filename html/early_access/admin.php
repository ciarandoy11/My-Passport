...
$otherActivity = isset($_POST['otherActivity']) ? 1 : 0;

// Database update if both item names are not empty
if (!empty($originalListName) && !empty($id)) {
    $stmt = $conn->prepare("UPDATE groups SET item_name = ?, list_name = ?, split = ?, pod_exemption = ?, pod_day_exemption = ?, day_exemption = ?, overide = ?, otherActivity = ? WHERE id = ?");
    if (!$stmt) {
        echo json_encode(["success" => false, "message" => "Failed to prepare update statement."]);
        exit;
    }

	$stmt->bind_param('sssiisiii', $UpdatedItemName, $UpdatedListName, $groupSplit, $podExemption, $podDayExemption, $dayExemption, $overide, $otherActivity, $id);
...
function fetchGroups($conn, $club) {
    ...
    $sql = "SELECT id, list_name, item_name, dob, inSchool, pod_exemption, pod_day_exemption, day_exemption, overide, split, otherActivity
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $split = $row['split'];
            $otherActivity = (int)$row['otherActivity'];
...
<main>
    ...
    <section>
        <div class='week-navigation' style='text-align: center;'>
            ...
            <button class="signOut" style="background-color: red; color: white; margin-top: 1%; margin-bottom: 10px;" onclick="document.location='login'">Sign Out</button>
            <button type="button" onclick="document.location='clubAttendance'" style="margin-top: 10px;background-color: #002061; color: #ffffff; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; font-size: 16px;">Attendance Reports & Coach's Signature</button>
        </div>
    </section>
                ...
                foreach ($days as $day) {
                    ...
                    if (isset($sessions[$day][$time])) {
                        ...
                        echo "<div class='session' draggable='true' data-id='$sessionId' ondragstart='drag(event)'>
                                ...
                                <p>POD: $pod</p>
                                <p>Coach: $coach</p>
                                <button style='min-height: 6px; min-width: 50px;' onclick='findPod($sessionId, \"$group\", \"$day\", \"$date\")'>Find Pod</button>
                              </div>";
                    }
                    echo "</td>";
                }
                ...
                <ul style="list-style-type: none; padding: 0;">
                    ...
                    <?php foreach ($items as $item): ?>
                        <form method="POST" class="item-form" style="border: 1px solid #ddd; padding: 10px; margin-bottom: 10px; border-radius: 5px;">
							...
							<div style="margin-top: 10px;">
                                <label for="otherActivity">Other Activity (E.g. Gym, Pilates, Talk, ect):</label>
                                <input type="checkbox" name="otherActivity" <?= !empty($item['otherActivity']) ? 'checked' : '' ?>>
                            </div>
...
<script>
   ...
   function findAllSessionPods() {
            ...
            console.log('Sessions to process:', sessions);
                    ...
                    try {
                        console.log(`Processing session ${session.sessionId}:`, session);
	                                const response = await findPod(session.sessionId, session.group, session.day, session.date);
    ...
    function showMessage(message, type) {
        ...
        messageContainer.style.opacity = '0.98';
        setTimeout(() => {
    ...
    async function findPod(sessionId, group, day, date) {
        try {
            ...
            const activityTypeResponse = await fetch('admin_actions.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
                body: new URLSearchParams({ 'action': 'get_activity_type', 'id': sessionId })
            });

            const activityType = await activityTypeResponse.json();
            ...
            console.log('Not exempt from POD:', acceptablePods);

            if (activityType['activityType'] === 'Other') {
                console.log('Fetching athletes/pods participating in other activities')
                const otherActivityAthletesResponse = await fetch('admin_actions.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded'
                    },
                    body: new URLSearchParams({ 'action': 'get_other_activity_athletes' })
                });
                var otherActivityAthletes = await otherActivityAthletesResponse.json();
                otherActivityAthletes = otherActivityAthletes['otherActivityAthletes'];
                console.log('Athletes/pods participating in other activities:', otherActivityAthletes);
            } else {
                otherActivityAthletes = acceptablePods;
            }

            // Filter out pods that are not participating in other activities to get the final available pods
            const activityAvailablePods = acceptablePods.filter(pod => otherActivityAthletes.includes(pod));
            
            console.log('Fetching pods exempt by date')
            const dateExemptionResponse = await fetch('admin_actions.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
                body: new URLSearchParams({ 'action': 'get_date_exemption', 'date': date })
            });
            var dateExemption = await dateExemptionResponse.json();
            dateExemption = dateExemption['dateExemption'];
            console.log(dateExemption);

            const finalAvailablePods = activityAvailablePods.filter(pod => !dateExemption.includes(pod));

            if (finalAvailablePods.length > 0) {
                console.log('Final available pods:', finalAvailablePods);
