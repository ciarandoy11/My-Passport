...
$sql = "SELECT * FROM fundraising WHERE id = ? AND club = ?";
...
<body>
    <main>
        ...
        <h2>Game: <?php echo htmlspecialchars($fundraisingGame['games']); ?></h2>
        ...
        <h2>Join the Game</h2>
        <form method="POST" action="donation_checkout.php?gameId=<?php echo $fundraisingId; ?>">
            <input type="hidden" name="fundraising_id" value="<?php echo $fundraisingId; ?>">
            <?php if ($fundraisingGame['games'] !== 'JustDonate'): ?>
                <label for="cards">Amount of <?php echo htmlspecialchars($fundraisingGame['games']); ?> cards?</label>
                <input type="number" name="cards" min="1" step='1'>
                <label for="amount">Optional Extra Donation Amount (in Euro):</label>
            <?php else: ?>
                <label for="amount">Your Donation (in Euro):</label>
            <?php endif; ?>
        ...
        <p><a href="fundraising">Back to Fundraising Games</a></p>
    </main>
</body>
</html>
