    ...
    if ($stmt->num_rows > 0) {
        ...
        if (password_verify($inputPassword, $hashedPassword)) {
            ...
            if (payment_verify($userId, $conn)) {
                $subscriptionTier = subscription_tier($userId, $conn) ? 'earlyAccess' : 'basic';
                $updateSql = "UPDATE users SET subscription_tier = ? WHERE id = ?";
                $updateStmt = $conn->prepare($updateSql);
                $updateStmt->bind_param("si", $subscriptionTier, $userId);
                $updateStmt->execute();
                $updateStmt->close();

                $subscriptionTier = ($subscriptionTier == 'earlyAccess') ? 'early_access' : $subscriptionTier;

                if ($typeAdmin == "1") {
                    header("Location: /" . $subscriptionTier . "/admin");
                    exit();
                } elseif ($typeCoach == "1") {
                    header("Location: /" . $subscriptionTier . "/coachs-site");
                    exit();
                } else {
                    header("Location: /" . $subscriptionTier . "/dashboard");
                    exit();
                }
            } else {
                $error = "Club subscription is not active.";
                if ($typeAdmin == "1") {
                    echo '<a href="clubSettings">Subscription Settings</a>';
                }
            }
        } else {
    ...
    <main>
        ...
        <form method="POST" action="">
            ...
            <div style="position: relative;">
                ...
                <a onclick="togglePassword()" style="position: absolute; right: 10px; top: 10px; height: 100%; line-height: 2.5; background-color: transparent; color: #007BFF; cursor: pointer; text-decoration: none;">Show</a>
            </div>
        ...
        <div class="links">
            <button onclick="window.location.href='reset_pw'">Forgot password?</button>
            <button onclick="window.location.href='signup'" style="background-color: #28A745">Don't have an account? Sign up</button>
        </div>
    </main>
