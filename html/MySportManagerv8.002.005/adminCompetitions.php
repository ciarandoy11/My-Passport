    ...
    switch ($fileType) {
        case 'hyv':
            ...
            for ($i = 1; $i < count($lines); $i++) {
                ...
                $eventInfo = explode(';', $line);
                if (count($eventInfo) >= 7) {
                    $event = [
                        'number' => trim($eventInfo[0]),
                        'name' => '', // Will be constructed from other fields
                        'age_group' => '',
                        'gender' => 'mixed',
                    ...
                    $event['name'] = $event['distance'] . ' ' . $event['stroke'];
                    if ($event['age_group']) {
                        $event['name'] .= ' ' . $event['age_group'];
                    }
                    $event['name'] .= ' ' . ucfirst($event['gender']);
                    ...
                    <tbody>
                        <?php while ($meet = $meets->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="viewMeet.php?id=<?php echo $meet['id']; ?>" class="button">View</a>
                                    <a href="meetResults.php?id=<?php echo $meet['id']; ?>" class="button">Results</a>
                                    <a href="meetEntries.php?id=<?php echo $meet['id']; ?>" class="button">Entries</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
