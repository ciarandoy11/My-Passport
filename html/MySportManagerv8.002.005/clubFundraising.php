			...
			<?php if ($result->num_rows > 0): ?>
				<?php while ($row = $result->fetch_assoc()): ?>
					<tr>
						...
						<td>
							<button onclick="if(confirm('Are you sure you want to delete this game?')) { window.location.href = 'deleteGameeId=<?php echo htmlspecialchars($row['id']); ?>'; }" style="background-color: #FF0000;">Delete</button>
						</td>
						<?php if ($row['games'] !== 'JustDonate'): ?>
							<td>
								<button onclick="window.location.href = 'runGameeId=<?php echo htmlspecialchars($row['id']); ?>';" style="background-color: #00FF00;">
									Run Game
								</button>
							</td>
						<?php endif; ?>
