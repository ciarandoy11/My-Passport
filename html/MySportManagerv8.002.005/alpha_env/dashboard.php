...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['info'])) {
        ...
        foreach ($swimmerArray as $currentSwimmer) {
            ...
            $updateGroupsSql = "UPDATE groups SET siblings = ?, sibling_index = ? WHERE item_name = ?";
            $updateGroupsStmt = $conn->prepare($updateGroupsSql);
            $updateGroupsStmt->bind_param("sis", $siblingsString, $siblingIndex, $currentSwimmer);
            $updateGroupsStmt->execute();
            $updateGroupsStmt->close();
        }
...
<main>
    ...
    <div id="info" class="card">
        ...
        <button class="button button-danger" onclick="document.location='login.php'">Sign Out</button>
        ...
        <p>Swimmer(s): <?php echo htmlspecialchars(!empty($user['swimmer']) ? $user['swimmer'] : 'Please put athlete(s) name here!!! Edit Info through the blue button below!!'); ?></p>
        <p>Group(s):
            <?php
            if (!empty($user['swimmer'])) {
                ...
                foreach ($swimmers as $index => $swimmer) {
                    ...
                    echo '<form action="" method="POST" class="card">
                            <li style="list-style-type: none; display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                                ...
                                ($inSchool === 1 ? ' checked' : '') . '>
                                <button type="submit" class="button button-success">Save DOB</button>
                            </li>
                        </form>';
                }
                echo '</ul>';
    ...
    <div id="editInfo" class="card" style="display: none;">
        <form method="POST" action="">
            ...
            <label for="Nswimmer">Swimmer(s) (comma separated):</label>
                                    ...
                                    <td>
                                        <form method="POST" action="">
                                            ...
                                            <?php } elseif ($isPodSession) { ?>
                                                    <button type="button" class="button" onclick="window.location.href='attendance.php?id=<?php echo $row['id']; ?>'">Take attendance for this session</button>
